...
struct GraphicsSettingsView: View {
        ...
        Form {
            ...
            Section(settings.localized("Filtering")) {
                ...
                if settings.casMode > 0 {
                    // Was a 0 to 1 float slider converting back to the stored percentage on every
                    // tick, which truncated. The value is a percentage, so drive it as one.
                    NumberRow("Sharpness", value: $settings.casSharpness,
                              in: SettingsStore.casSharpnessRange, format: .percent,
                              settings: settings)
                }
            ...
            Section {
                ...
                if settings.shadeBoost {
                    shadeBoostRow("Brightness", value: $settings.shadeBoostBrightness)
                    shadeBoostRow("Contrast", value: $settings.shadeBoostContrast)
                    shadeBoostRow("Saturation", value: $settings.shadeBoostSaturation)
                    shadeBoostRow("Gamma", value: $settings.shadeBoostGamma)
                }
            } header: {
            ...
            Section(settings.localized("Advanced Upscaling Hacks")) {
                ...
                NumberRow("Texture Offset X", value: claiming("UserHacks_TCOffsetX", $settings.textureOffsetX),
                          in: SettingsStore.textureOffsetRange, style: .field, settings: settings)
                NumberRow("Texture Offset Y", value: claiming("UserHacks_TCOffsetY", $settings.textureOffsetY),
                          in: SettingsStore.textureOffsetRange, style: .field, settings: settings)
            ...
            Section {
                ...
                NumberRow("CPU Sprite Render BW", value: $settings.cpuSpriteRenderBw,
                          in: SettingsStore.cpuSpriteRenderBwRange, style: .field, settings: settings)
                NumberRow("CPU Sprite Render Level", value: $settings.cpuSpriteRenderLevel,
                          in: SettingsStore.cpuSpriteRenderLevelRange, style: .field, settings: settings)
    ...
    private func shadeBoostRow(_ title: String, value: Binding<Int>) -> some View {
        NumberRow(title, value: value, in: SettingsStore.shadeBoostRange,
                  format: .percent, default: 50, settings: settings)
    }
}
