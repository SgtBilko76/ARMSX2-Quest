...
That is not hypothetical. The Tile renderer shipped five of its seven translation units
and six of its headers absent from the project file, and was unbuildable on Windows from
M2 until 2026-08-15 -- caught only because someone tried a configuration this box cannot
compile. Eight more sources were still missing after that was fixed, five of them
nowhere near the GS.
...
def main():
	...
	in_project = vcxproj_files(vcx_path)
	base = cmake_path.parent

	missing = []
	checked = 0
	for name in sorted(UNCONDITIONAL_LISTS):
		for rel in entries.get(name, []):
	...
	if missing:
		...
		print(f"pcsx2.vcxproj does not name {len(missing)} file(s) that CMake builds on "
			  f"every platform ({len(srcs)} of them sources):")
		for rel, name in sorted(missing, key=lambda m: (not m[0].endswith(CODE_SUFFIXES), m[0])):
			kind = "SOURCE" if rel.endswith(CODE_SUFFIXES) else "header"
			print(f"    {kind:6}  {rel}    [{name}]")
		print("\n  A missing SOURCE is a Windows link failure. A missing header only hides")
		print("  the file from the IDE. Add each to pcsx2.vcxproj AND pcsx2.vcxproj.filters.")
	else:
		print(f"ok    msvc file list: pcsx2.vcxproj names all {checked} unconditional "
			  f"CMake files")
